...
class DataConfig:
    ...
    数据配置类，用于存储数据处理相关的参数
    """
    meta_data: str = "/home/fsq/AlignLLM4Code_GRPO/code/raw_data/75k/lcs_split_data/comment_lcs_split_results.jsonl"  # 元数据CSV文件路径
...
#         example, data_dir, eval_dims='VQ', max_pixels=448 * 448, fps=2.0,
...
def convert_anno_csv_to_reward_data(
    ...
    data = [
        {
            "role": "user",
            "content": build_prompt(eval_dims, example['code-instruction'], example['answer'])
        }
    ]
    ...
    # score = torch.tensor(example['final_score'], dtype=torch.float)
    # 不知道这个fianl_score需不需要先处理一下，它包含小数
...
def create_dataset(data_config: DataConfig, meta_file=None):
    ...
    #     example, data_config.data_dir, data_config.eval_dim
    # )
    convert_func = lambda example : convert_anno_csv_to_reward_data(
